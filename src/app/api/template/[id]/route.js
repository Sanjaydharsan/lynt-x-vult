...
const parseJsonFields = (item) => {
  ...
  if (parsedItem.icon) {
    try {
      ...
      if (typeof parsedItem.icon === 'string' && !parsedItem.icon.startsWith('{') && !parsedItem.icon.startsWith('[')) {
        // It's a plain string like "FileText", keep as is
        parsedItem.icon = parsedItem.icon;
      } else {
      ...
      parsedItem.icon = parsedItem.icon; // Keep original value if parsing fails
    }
  }
  if (parsedItem.properties) {
    try {
      if (typeof parsedItem.properties === 'string') {
