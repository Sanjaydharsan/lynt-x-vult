...
export default function FileUploader({ batchName, onUploadComplete }) {
  ...
  const { mutate: createOrders } = useCreateOrders();
  ...
  const uploadFiles = async () => {
    ...
    createOrders(formData, {
      onSuccess: (data) => {
        ...
        setFiles((prev) =>
          prev.map((f) => {
            const uploaded = data.uploaded?.find((u) => u.imagename === f.name);
            const skipped = data.skipped?.find((s) => s.imagename === f.name);
            return {
              ...f,
              status: uploaded ? "uploaded" : skipped ? "skipped" : "unknown",
            };
          })
        );
        setResponse(data);
      ...
      <div className={`p-6 ${colors.background} rounded-3xl w-full`}>
        ...
        >
          ...
          <p className={`text-sm ${colors.textSecondary}`}>
            PDF, PNG, JPG, JPEG, TIF up to 10MB
          </p>
        </div>
        ...
        {files.length > 0 && (
          <div className="mt-6 flex justify-end space-x-3 w-full">
            <button
              ...
              className={`px-4 py-2 text-sm font-medium rounded-md transition-colors cursor-pointer ${colors.buttonSecondary}`}
            >
              Cancel
            </button>
            <button
              onClick={uploadFiles}
              disabled={uploading}
              className={`px-4 py-2 text-sm font-medium text-white rounded-md transition-colors cursor-pointer flex items-center gap-2 ${
                uploading
        ...
        {files.length > 0 && response && (
          <div className="mt-4 w-full space-y-2">
            {response.success && response.uploaded?.length > 0 && (
              <div className="p-3 rounded-lg bg-[#e6f4ea] border border-green-400 text-green-800">
                <p>✅ Upload complete!</p>
                <p>{response.uploaded.length} file(s) uploaded successfully.</p>
              </div>
            )}

            {response.skipped && response.skipped.length > 0 && (
              <div className="p-3 rounded-lg bg-yellow-100 border border-yellow-400 text-yellow-900">
                <p>⚠️ Some files were skipped:</p>
                <ul className="list-disc pl-5">
                  {response.skipped.map((file) => (
                    <li key={file.imagename}>{file.message}</li>
                  ))}
                </ul>
              </div>
            )}
            ...
            {response.error && (
              <div className="p-3 rounded-lg bg-[#fce8e6] border border-red-400 text-red-800">
                <p>❌ {response.error}</p>
              </div>
            )}
          </div>
