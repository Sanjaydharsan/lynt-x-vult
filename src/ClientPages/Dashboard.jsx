...
export default function Dashboard() {
  ...
  const { organization, isLoading:organizationLoading, isError } = useGetOrganization(createdby);
  const isLoading =
...
useEffect(() => {
  if (organization && !isLoading && !isError) {
    console.log("Sync method", organization.syncmethod)
    setFromEmail(organization.fromemail);
 
    const methods = organization.syncmethod
      ? organization.syncmethod.split(',').map((m) => m.trim())
      : [];
    setSyncMethods(methods);
  }
 ...
 useEffect(() => {
  if (
    !isLoaded ||
    !syncMethods.includes("email_synchronization") ||
    !fromEmail
  ) {
    return;
  }
  ...
  const runSync = () =>
    startTransition(() => {
      fetchAndUpload({ fromEmail, org_id });
    });
 
  runSync();
  const intervalId = setInterval(runSync, 2 * 60 * 1000);
  return () => clearInterval(intervalId);
}, [isLoaded, syncMethods, fromEmail]);

 useEffect(() => {
  if (
    !isLoaded ||
    !syncMethods.includes("xero") ||
    !organization
  ) {
    return;
  }
  ...
  const runSync = () =>
    startTransition(() => {
      handleSyncXero(organizationId);
    });
 
  runSync();
  const intervalId = setInterval(runSync, 5 * 60 * 1000);
  return () => clearInterval(intervalId);
}, [isLoaded, syncMethods, organization]);
  
  useEffect(() => {
    if (!isLoaded || !syncMethods.includes("zoho") || !organization) {
      return;
    }
                                ...
                                <Drawer direction="right">
                                  <DrawerTrigger asChild>
                                    <button
                                      ...
                                      disabled={!pdfjsLib}
                                      title={!pdfjsLib ? "PDF.js is loading..." : "View Image"}
                                    >
                                      <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        viewBox="0 0 576 512"
                                      ...
                                      <span>{!pdfjsLib ? "Loading..." : "View Image"}</span>
                                    </button>
                                  </DrawerTrigger>
                                  <DrawerContent className="custom-drawer-content bg-white rounded-l-xl shadow-xl p-6 overflow-y-auto">
                                    ...
                                    {isConverting ? (
                                      ...
                                      <p className="text-gray-600">
                                        {!pdfjsLib ? "PDF.js is loading..." : "No image available."}
                                      </p>
                                    )}
                                  </DrawerContent>
                                </Drawer>
